...
export function r21PatientSummary(filters: Filters): ReportQuery {
...
export function r22HqExpenses(filters: Filters): ReportQuery {
  const { where, params } = parseFilters(filters);
  return {
    text: `
...
export function r23Dcpp(filters: Filters): ReportQuery {
  const { where, params } = parseFilters(filters);
  return {
    text: `
