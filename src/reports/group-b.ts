...
export function r6ItemWise(filters: Filters): ReportQuery {
...
export function r7ItemHqPerformance(filters: Filters): ReportQuery {
  const { where, params } = parseFilters(filters);
  return {
    text: `
...
export function r8ItemFyIncrDecr(_filters: Filters): ReportQuery {
  return {
    text: `
      SELECT item_name, fy,
        SUM(net_sales_) AS net_primary
...
export function r9ItemMonthly(filters: Filters): ReportQuery {
  const { where, params } = parseFilters(filters);
  return {
    text: `
...
export function r10ItemReturn(filters: Filters): ReportQuery {
  const { where, params } = parseFilters(filters);
  return {
    text: `
